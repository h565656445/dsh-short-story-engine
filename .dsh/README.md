...
将 `.dsh` 目录复制到 `$DSH_HOME/.agent-presets/short-story-engine/`：

```powershell
# Windows PowerShell 7
$dst = Join-Path $env:DSH_HOME ".agent-presets\short-story-engine"
Copy-Item -Recurse -Force ".\dsh" $dst
```

重启 DeepSeek Harness 后，在会话预设列表选择 `短篇小说状态机`。

### 方式二：仅安装技能（skill）

将 SKILL.md 复制到 `$DSH_HOME/skills/dsh-short-story-engine/`：

```powershell
$dst = Join-Path $env:DSH_HOME "skills\dsh-short-story-engine"
New-Item -ItemType Directory -Force -Path $dst | Out-Null
Copy-Item ".\skills\dsh-short-story-engine\SKILL.md" $dst -Force
```
